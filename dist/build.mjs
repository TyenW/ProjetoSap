...
let MINIFY_ENABLED = true;
try {
  postcss = (await import('postcss')).default;
  cssnano = (await import('cssnano')).default;
  terser = await import('terser');
} catch (err) {
  // Fall back to copy-only build if dev deps are missing (e.g., Vercel prod install)
  MINIFY_ENABLED = false;
  console.warn('[build] Minification disabled (postcss/cssnano/terser not installed).');
  console.warn('[build] Proceeding with copy-only build. To enable minification locally, run:');
  console.warn('        npm install --save-dev postcss cssnano terser');
}
...
async function minifyCss(content) {
  if (!MINIFY_ENABLED || !postcss || !cssnano) return content;
  const result = await postcss([cssnano()]).process(content, { from: undefined });
  return result.css;
}

async function minifyJs(content) {
  if (!MINIFY_ENABLED || !terser || !terser.minify) return content;
...
async function build() {
  console.log(`Building ProjetoSap → dist/ ${MINIFY_ENABLED ? '(minify enabled)' : '(copy-only)'}`);
  await cleanOut();
